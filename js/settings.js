...
async function changePassword() {
    const newPassword = prompt('Въведи нова парола (поне 6 символа):', '');
    ...
    try {
        ...
        showNotification(err.message || 'Грешка при смяна на парола!', 'error');
    }
}

async function deleteAccount() {
    if (!confirm('⚠️ Сигурен ли си, че искаш да изтриеш своя профил? Това действие не може да бъде отменено.')) return;
    try {
